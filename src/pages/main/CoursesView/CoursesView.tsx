...
const CoursesView = () => {
  ...
  useEffect(() => {
    if (isSuccess && data?.data) {
      setCourse(data.data);
      setVideo(getMediaUrl(data.data.trailer));
    }
  }, [data, isSuccess]);
  ...
  return (
    <div className="pt-4 pb-5 bg-gradient-to-r from-[#090913] to-[#0d0d15] text-white">
      <Container>
        {course ? (
          <div>
            <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-3 ">
              <div className="h-max p-3 lg:p-6 rounded-lg lg:rounded-xl border border-[#CDC9C9] lg:col-span-2">
                ...
                <CourseCover image={course.image} trailer={video} />
                <div className="flex justify-between items-center gap-2 text-black">
                  {["Overview", "Teachers", "Routine", "Review"].map((item) => (
                    <CourseTabBtn
                      key={item}
                      name={item}
                      tab={tab}
                ...
                <div className="grid gap-2 text-xs lg:text-sm">
                  <div className="flex justify-between items-center gap-2 pb-1 rounded border-b border-[#CDC9C9]">
                    <p className="flex items-center gap-1 lg:gap-1.5">
                      <FaRegClock className="size-3 lg:size-4" /> Course
                      Duration
                    </p>
                    <p>
                      {course.duration[0]} {course.duration[1]}
                    </p>
                  </div>
                  {course?.details?.totalClasses && (
                    <div className="flex justify-between items-center gap-2 pb-1 rounded border-b border-[#CDC9C9]">
                      <p className="flex items-center gap-1 lg:gap-1.5">
                        <IoBookOutline className="size-3 lg:size-4" /> Total
                        Classes
                      </p>
                      <p>{course.details.totalClasses}+</p>
                    </div>
                  )}
                  {course?.details?.totalLiveClasses && (
                    <div className="flex justify-between items-center gap-2 pb-1 rounded border-b border-[#CDC9C9]">
                      <p className="flex items-center gap-1 lg:gap-1.5">
                        <IoVideocamOutline className="size-3 lg:size-4" /> Live
                        Classes
                      </p>
                      <p>{course.details.totalLiveClasses}+</p>
                    </div>
                  )}
                  {course?.details?.totalLectures && (
                    <div className="flex justify-between items-center gap-2 pb-1 rounded border-b border-[#CDC9C9]">
                      <p className="flex items-center gap-1 lg:gap-1.5">
                        <IoTvOutline className="size-3 lg:size-4" /> Recorded
                        Classes
                      </p>
                      <p>{course.details.totalLectures}+</p>
                    </div>
                  )}
                  {course?.details?.totalNotes && (
                    <div className="flex justify-between items-center gap-2 pb-1 rounded border-b border-[#CDC9C9]">
                      ...
                      <p>{course.details.totalNotes}Pcs</p>
                    </div>
                  )}
                  {course?.details?.totalExams && (
                    <div className="flex justify-between items-center gap-2 pb-1 rounded border-b border-[#CDC9C9]">
                      <p className="flex items-center gap-1 lg:gap-1.5">
                        <FaQuestion className="size-3 lg:size-4" /> Total Exams
                      </p>
              ...
              <div className="lg:col-span-2 py-3 lg:py-6">
                <CourseTabContent key="Overview" name="Overview" tab={tab}>
                  <div>
                    ...
                    <div>
                      <Curriculum courseID={course._id} setVideo={setVideo} />
                    </div>
                  </div>
                </CourseTabContent>

                <CourseTabContent key="Teachers" name="Teachers" tab={tab}>
                  <div>
                    <h4 className="text-xl lg:text-2xl font-bold mb-3">
                      Instructors
                    </h4>
                ...
                <CourseTabContent key="Routine" name="Routine" tab={tab}>
                  <div>
                    <h4 className="text-xl lg:text-2xl font-bold mb-3">
                      Routine
                    </h4>
                    <div className="flex justify-end">
                      {course?.routinePDF && (
                        <a href={course.routinePDF} target="_blank">
                          Download Routine
                        </a>
                      )}
                    </div>
                ...
                <CourseTabContent key="Review" name="Review" tab={tab}>
                  <ReviewSection course={course} isMyCourse={isMyCourse} />
                </CourseTabContent>
              </div>
            </div>
          </div>
